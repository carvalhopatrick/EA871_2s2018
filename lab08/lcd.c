...
void putcmd_LCD(unsigned char instr, unsigned int t)
{
	...
	delay(200);					// Espera para evitar outro enable em tempo < que tcycE
}
...
void init_LCD(void)
{
	...
	putcmd_LCD(0b00001100, WAIT);	// ON/OFF control: D = 1, C = 1, B = 1 
	clear_LCD();
...
void setpos_LCD(unsigned int linha, unsigned int coluna)
{
	...
	if (linha == 1)
	{
		putcmd_LCD((0x80 + coluna), 200);
	}
	else if(linha == 2)
	{
		putcmd_LCD((0xC0 + coluna), 200);
	}
}
