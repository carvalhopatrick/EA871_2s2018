/* 			EA869 - Roteiro 5
	Patrick Penacho Carvalho	185388
				...
				lcd.c
	Funções relacionadas ao módulo LCD 	*/
...
/* Função puts_LCD */
void puts_LCD(unsigned char *str)
{
	int i = 0;
	setpos_LCD(1,0);		// Começa com o cursor na 1a posição da 1a linha
	
	while(str[i] != '\0'   &&   i<40) 	// Escreve na primeira linha até fim da string ou fim definitivo da linha (pos. 47)
	{
		putchar_LCD(str[i]);
		if (i<31)
		{
			delay(600000);	// Nao tem delay entre escrita para caracteres "invisiveis"
		}
		i++;
	}
	
	if (!(str[i] == '\0'   &&   i<16)) 	// nao roda se string é menor que 16 carac.
	{
		i = 16;				// Volta para caracter seguinte ao ultimo visivel na primeira linha
		setpos_LCD(2,0); 	// 2a linha
		
		while(i<=31 && str[i] != '\0') // Escreve até lotar parte visivel da 2a linha
		{
			putchar_LCD(str[i]);
			delay(600000);
			i++;
		...
		while(str[i] != '\0')
		{
			putcmd_LCD(0b00011000,WAIT);	// habilita shift do display
			putchar_LCD(str[i]);			// escreve prox. caracter
			delay(700000);
			i++;
		}
	}
